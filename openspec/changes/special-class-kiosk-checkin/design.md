...
- **F2 (card walk-in)**: kiosk sends nothing that triggers the special branch. Point the kiosk QR at the existing reservation page instead of the generic checkout session — the customer self-enters name/email/phone there, which is exactly the web flow already verified end-to-end. This sidesteps the `attemptId` (UUID) + email hard requirement (`session/route.ts:58-64`) entirely. The configured Special Salsa QR MUST use `/special-classes/${SPECIAL_SALSA_CLASS.key}`; `PublicSpecialClass` preserves its generic interface while reusing the shared phone domain's international country catalog and E.164 normalization.
- **Generic reservation rendering**: `PublicSpecialClass` MUST format event instants with `formatSpecialClassDateTime`, which uses the authoritative `SPECIAL_SALSA_CLASS.timeZone` (`America/New_York`), so server and browser render the same event time.
