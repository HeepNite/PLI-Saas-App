...
The mobile video startup correction supersedes the earlier blurred-backdrop instruction: the hero renders exactly one accessible foreground video decoder/source. That video uses `object-contain` and is never cropped. A non-video visual background may fill the media panel behind it, but it must not load, decode, synchronize, or otherwise render a second video. The foreground retains its poster, muted looping inline behavior, label, and accessible play/pause and sound controls. It must not use native `autoPlay`; after client-side confirmation that `prefers-reduced-motion` is not requested it may begin playback programmatically. When reduced motion is requested it starts paused, while an explicit user Play action remains available and works.

The final hero-overlay refinement supersedes the earlier separated bottom placements: `60 min` and `40 spots` remain left-aligned in the same bottom row, vertically centered with the right-aligned play/pause and sound controls. Both controls remain visible and keyboard operable. The contrast gradient is limited to the lower quarter of the media panel with reduced opacity so the instructor and partner remain visible in the bottom of the uncropped portrait foreground.
...
The page must be usable without horizontal scrolling at 375, 768, 1024, and 1440 CSS pixels. Dialog form controls require associated labels, errors must be announced, focus must move to the first invalid field or outcome message, and all interactive controls must be keyboard reachable with visible focus. Focus must not escape an open reservation dialog, and the background page must not scroll beneath it. No global floating control may overlap the special landing's dialog inputs or purchase actions. The hero must render one accessible foreground video with an accessible name, poster/fallback, muted looping inline playback, and uncropped `object-contain` presentation. It must not render a second video decoder/source; any panel background is non-video. It must not use native `autoPlay`: only after client-side confirmation that reduced motion is not requested may it attempt programmatic playback. Reduced-motion users start paused but can explicitly start playback from the clearly visible, keyboard-operable in-page play/pause button, which announces its current action and pressed state. An adjacent always-visible sound button must start in the muted state, unmute or mute the same video only after user interaction, and announce its current action and pressed state. If programmatic playback is rejected, the play button remains available. Hover and focus states must not shift layout. Information required to purchase must also exist as text and must not depend on video audio.
