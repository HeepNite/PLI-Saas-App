...
- [x] add explicit lifecycle support for `ACTIVE`, `SUSPENDED`, `SCHEDULED`, and `DELETED`
- [x] add `launchAt` or equivalent scheduled-launch field
...
- [x] ensure current public availability logic continues to hide non-`ACTIVE` packages while admin can still manage lifecycle states

## Phase 4 - Admin UI Refactor

- [x] refactor the package management list into a denser row/table layout
- [x] display package name, course, price, lifecycle status, and launch date in each row
- [x] add explicit row actions for edit, duplicate, suspend/activate, and delete
...
- [x] add scheduling controls for launch date and lifecycle state transitions

## Phase 5 - Public Visibility And Read Path Adjustments

- [x] update package read/query filters so only eligible `ACTIVE` packages are public
...
- [x] add unit tests for lifecycle transition rules (`ACTIVE`, `SUSPENDED`, `SCHEDULED`, `DELETED`)
