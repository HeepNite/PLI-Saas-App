...
export default function CheckInQrClient({
  ...
  const showKioskPinPanel =
    mode === "existing" && isKioskTerminalFlow && !isSignedIn && (!hasKioskPinSession || kioskPinRotationRequired)
  ...
  const showKioskResolvingOverlay = shouldShowKioskResolvingOverlay({
    isKioskTerminalFlow,
    mode,
    hasActiveCustomerSession: Boolean(isSignedIn || hasKioskPinSession),
    loadingBootstrap,
          ...
          {showKioskPinPanel && (
            <div className="fixed inset-0 z-[12000] flex items-start justify-center bg-black/72 px-4 pb-4 pt-28 backdrop-blur-sm sm:px-6 sm:pt-32 md:pb-6 md:pt-36 lg:items-center lg:pt-10">
              <div
                ...
                className="max-h-[calc(100vh-7rem)] w-full max-w-[28rem] overflow-y-auto rounded-[1.75rem] border border-white/10 bg-[radial-gradient(circle_at_top_right,rgba(182,22,22,0.18),transparent_54%),linear-gradient(160deg,rgba(12,15,28,0.99),rgba(21,25,40,0.97))] shadow-[0_24px_60px_-32px_rgba(0,0,0,0.85)] md:max-w-[46rem]"
              >
                <div className="grid gap-4 p-4 sm:p-5 md:grid-cols-[minmax(0,1fr)_18rem] md:gap-5 lg:grid-cols-[minmax(0,1fr)_19rem]">
                  <div className="rounded-[1.5rem] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.06),rgba(255,255,255,0.02))] p-4 sm:p-5">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="text-xs uppercase tracking-[0.16em] text-[var(--brand,#d23434)]">
                    ...
                    {!hasKioskPinSession ? (
                      <>
                        ...
                        <button
                          ...
                          className="mt-5 w-full rounded-xl bg-[var(--brand,#b61616)] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[var(--brand-strong,#991010)] disabled:opacity-50"
                        >
