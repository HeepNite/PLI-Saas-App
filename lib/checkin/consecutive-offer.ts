 ...
 *
 * This eliminates the N+1 query pattern from the bootstrap route's inline logic.
 */
export const resolveConsecutiveOffer = async ({
  ...
  // Collect all linked course slugs for batch lookup
  ...
  // Batch: single query for all linked courses instead of N×getCatalogCourseBySlug
  ...
  // Batch: single query to check which slugs already have a purchase today
  ...
  // For the offer we need the full CourseData for enrollment pricing.
  // Fall back to getCatalogCourseBySlug which fetches package plans too.
