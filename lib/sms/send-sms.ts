 ...
 *
 * When Twilio env vars are not configured (local dev, or a rollout window
 * before keys are provisioned), `sendSms` degrades to an inert no-op that
 * NEVER throws — callers can always safely `await sendSms(...)`.
 */
